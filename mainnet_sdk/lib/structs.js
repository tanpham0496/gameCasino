...
class Tx {
    constructor(habrosRatio, habrosLimit) {
        this.habrosRatio = habrosRatio;
        this.habrosLimit = parseInt(habrosLimit);
        this.actions = [];
        this.signers = [];
        this.signatures = [];
        this.publisher = "";
    ...
    setHabros(habrosRatio, habrosLimit) {
        if (typeof habrosLimit !== 'number' || habrosLimit > 4000000 || habrosLimit < 6000) {
            throw "habros limit should be in [6000, 4000000]"
        }
        if (typeof habrosRatio !== 'number' || habrosRatio < 1 || habrosRatio > 100) {
            throw "habros limit should be in [1, 100]"
        }

        this.habrosLimit = habrosLimit;
        this.habrosRatio = habrosRatio
    }
    ...
    _bytes(n) {
        ...
        c.pushInt64(parseInt(this.habrosRatio * 100));
        c.pushInt64(this.habrosLimit * 100);
