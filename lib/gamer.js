...
Gamer.prototype.onLogout = function() {
	...
	if(gamer.room) {
		...
		try{
			...
			userService.updateDataRoomCasino({roomNum : config.Room_Id, userId : gamer.uid, status : 'waiting', typeGame : config.typeGame, join : 0});
		}
		catch(e){
			console.log('Error', e);
		}
		...
		setTimeout(() => {
			db.hgetall(room_key, function(err, ret) {
				...
				if(ret && ret.gamers_count >= '6'){
					console.log('Room is full! Please come back later ======>');
					console.log('gamer.uid', gamer.uid);
					gamer.socket.emit('Res_Room_Full',{game : 'poker', uid : gamer.uid});  
					func(400, 'Room is full! Please come back later'); return;
				}else{
					try{
						gamer.socket.join(room_key);
						userService.updateDataRoomCasino({roomNum : roomid, userId : gamer.uid, status : 'waiting', typeGame : config.typeGame, join : 1})	
					}
					catch(e){
						console.log(e);
...
Gamer.prototype.onGamer_leave = function(req, func) {
	...
	(async()=> {
		const data = await userService.updateDataRoomCasino({roomNum : config.Room_Id, userId : gamer.uid, status : 'waiting', typeGame : config.typeGame, join : 0});
		if(data){
