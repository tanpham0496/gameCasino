...
HwatuGame.prototype.gameOver = function() {
	...
	if(room.next_match) {
		...
		for(i=0; i<in_gamers.length; i++) {
			gamer = in_gamers[i];
			if(gamer.nextGame) {
				console.log('{uid ==== chips }', {uid :  gamer.profile.uid, chips : gamer.chips})
				gamer.profile.coins = gamer.profile.coins + gamer.chips;
			
				userService.TxCreateRewardHwatu({ amount : gamer.chips, userId : gamer.profile.uid }).then(result=> {
					// console.log('Dra =====>==========================>function Gameover', result)
				})
				listBettingInGame.push({uid : gamer.profile.uid, coins : gamer.chips, draw : 'true'});
				gamer.saveData();
			}
			else{
		...
		// 							userService.TxCreateRewardHwatu({ amount : loser.chips, userId : gamer.profile.uid }).then(result=> {
		// 								// console.log('loser =====>==========================>function Gameover', result)
		// 							});

		// 							listBettingInGame.push({uid : gamer.uid, coins : gamer.chips});
		// 							gamer.profile.exp ++;
		// 							gamer.saveData();
		// 						}
		// 					})
		...
		// 						gamer.profile.coins = gamer.profile.coins + (resolve.totalOnMatch)*percentReceive;
		// 						//send money admin to user
		// 						room.notify(gamer.profile.uid, 'sendMoneyToWinner', {money : resolve.totalOnMatch, nameGame : 'hwatu'});
