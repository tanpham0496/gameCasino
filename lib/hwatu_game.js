...
HwatuGame.prototype.gameOver = function() {
	...
	if(room.next_match && in_gamers){

		console.log('room.next_match && in_gamers.length ==============', in_gamers)
	...
	var param = {
		roomId : room.id,
		in_gamers : inGamers,
		Winner : gamerWin && gamerWin[0].uid ? gamerWin[0].uid : 'Leave_Room',
		status : "endGame",
		type : 'hwatu',
						...
						for(i=0; i<in_gamers.length; i++) {
							...
							if(gamer.prize > 0) {
								...
								if(gamer.cards.length == 2 && room.shared_cards.length ==5){
									...
									console.log('-------------------------------')
									console.log('--gamer.cards-----------------------', gamer.cards)
									// nameCardOfWinnerHwatu = HwatuPoker.Gwangdang(gamer.cards);
									nameCardOfWinnerHwatu =  HwatuPoker.rank( gamer.cards );
...
HwatuGame.prototype.moveTurnToNext = function() {
	...
	if(typeof last === 'undefined') {
		...
		do {
			const firstElement = in_gamers.shift();
			console.log('------ firstElement ------', firstElement.uid)
			in_gamers.push( firstElement );
			
			next = in_gamers[0];
			console.log("typeof next",typeof next);
			console.log("-------------NEXT--------------------->", next.uid);
			console.log("-------------seat === room.first_turn--------------------->", next.seat);
			console.log("-------------room.first_turn--------------------->", room.first_turn);
...
HwatuGame.prototype.gamerShowDown = function() {
	...
	if(location.message === 'newGame'){
		...
		room.notifyAll('showNameCardWinner', {nameCardOfWinnerHwatu : 'There Player have same score ', cause : 'that will play game one more'});
			...
			for(let i = 0; i <room.in_gamers.length ; i ++){
				if(listWinner.includes(room.in_gamers[i])){
					tmp++;
				}
			}
			console.log('+++++++ ====> tmp=======>', tmp);
			...
			if(tmp == 0){
				for( let i = 0; i < room.in_gamers.length ; i++){
					var gamerNewReady = room.in_gamers[i];
					room.pub.publish('room:#'+ room.id, JSON.stringify(
						{ 
							uid: gamerNewReady.uid, 
							f: 'ready', 
							args: '0' 
						}
					));
				...
				for( let i = 0; i < room.in_gamers.length ; i++){
					...
					listWinner.map(item => {
						...
						if(item.uid === gamerNewGame.uid){
							room.ReGameListUser.push(gamerNewGame.uid);
		...
		const saveData = blocksCasino.createBlocksCasino(param);
		room.notifyAll('showNameCardWinner', {nameCardOfWinnerHwatu : 'The One of the players have score 40 || 30', cause : ' that will play game again!'});
		// =============>
		...
		if( room.in_gamers.length == 0 ||  room.in_gamers.length == 1){
		...
		else if( room.in_gamers.length == 1 ){
			for( let i = 0; i < room.in_gamers.length ; i++){
				var gamerNewReady = room.in_gamers[i];
				room.pub.publish('room:#'+ room.id, JSON.stringify(
					{ 
			...
			for( let i = 0; i < room.in_gamers.length ; i++){
				...
				if(!room.listFold.includes(gamerReGame.uid)){
					...
					room.ReGameListUser.push(gamerReGame.uid);
				}
				else{
