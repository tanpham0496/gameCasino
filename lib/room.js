...
Room.prototype.onMessage = function(message) {
	...
	try {
		// console.log('Room______onMessage*******_____',message);
...
//add new code
Room.prototype.notifyUserId = function (event, args, listUserId) {
	listUserId.map(item => {
		this.pub.publish('user:#' + item.uid, JSON.stringify({
			f : 'event',
			uid : item.uid,
			e : event,
			args : args
...
Room.prototype.enableButtonLeaveOnRoom = function (event, args, listUserId) {
	listUserId.map(itemId => {
		this.pub.publish('user:#' + itemId, JSON.stringify({
			f : 'event',
			uid : itemId,
			e : event,
			args : args
...
Room.prototype.notifyWinner = function(userWin, event, args) {
	var pub = this.pub;
	var gamers = this.gamers;
	for(var i in gamers) {
...
Room.prototype.autoLeave = function(uid, event, args) {
	console.log('====>autoleave<=======', {uid, event, args})
	this.pub.publish('user:#' + uid, JSON.stringify({
		f : 'leave',
		uid : uid,
		e : event,
