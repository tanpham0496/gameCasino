...
Casino.prototype.startInstance = function( instance_id ) {
	this.id = instance_id = config.Room_Id;
	
	var casino = this;
	var conf = this.conf;

	var now = Date.now();
	var key = 'casino:#' + config.Room_Id;
	var db = this.db;
	db.multi()
		.hmset(key, {
			id: config.Room_Id,
		...
		.expire(key, 5)
		.zadd('casino:all', now, config.Room_Id)
		.exec();
	...
	sub.on('message', function(channel, message){
		console.log('message ______________',channel, message);
	...
	sub.subscribe('casino:#'+config.Room_Id);

	this.is_running = true;
	this.pub.publish('server:log', 'casino #' + config.Room_Id + ' started');
};
...
Casino.prototype.startup = function( conf ) {
	...
	casino.startInstance(config.Room_Id);
...
Casino.prototype.shutdown = function() {
	...
	for(var i in rooms) {
		var room = rooms[i];
		var roomid = room.id;
		m.del('room:#' + config.Room_Id )
		 .zrem('game:#'+room.type+'#rooms', config.Room_Id)
		 .zrem('room:all', config.Room_Id);
	...
	m.del('casino:#' + config.Room_Id)
	.zrem('casino:all', config.Room_Id)
	...
	this.pub.publish('server:log', 'casino #' + config.Room_Id + ' stopped');
...
Casino.prototype.addGameType = function( typeid, conf ) {
	...
	// console.log('require**game============>',conf.game);
...
Casino.prototype.addRoom = function(typeid, n) {
	...
	var func = function(err,roomid){
		if(err) return;
		var room = new Game( casino, typeid, config.Room_Id, options );
		room.setName(gametype.name);
		
		gametype.rooms[ config.Room_Id ] = room;
		gametype.rooms_count ++;
		
		casino.rooms[ config.Room_Id ] = room;
		casino.rooms_count ++;
		
		var room_key = 'room:#' + config.Room_Id;
		db.multi()
			.zadd('game:#' + typeid + '#rooms', now, config.Room_Id)
			.zadd('room:all', now, config.Room_Id)
...
Casino.prototype.tick = function(){
	...
	for(var roomid in rooms) {
		var room = rooms[ config.Room_Id ];
		
		var room_key = 'room:#' + config.Room_Id;
		
		m.expire(room_key, 5)
		 .zadd('room:all', now, config.Room_Id)
		 .zadd('game:#' + room.type + '#rooms', now, config.Room_Id)
		...
		if(room.gamers_count < room.seats_count) {
			m.zadd('game:#' + room.type + '#rooms_notfull', now, config.Room_Id);
		}
	}
	...
	m.expire('casino:#' + config.Room_Id, 5)
	 .zadd('casino:all', now, config.Room_Id)
