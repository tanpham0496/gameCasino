...
router.post('/sipping',sipping);
router.post('/sippingBetting',sippingBetting);
router.post('/srp',srp);
router.post('/srpBetting',srpBetting);
router.post('/lotto',lotto);
router.post('/userBalance', userBalance);
router.post('/lottoBetting', lottoBetting);

// router.get('/api', getDocumentApi);
router.get('/checkSchedule', checkSchedule);
router.get('/runSchedule', runSchedule);
router.get('/getTimestamp', getTimestamp);

router.post('/tx-create-reward', TxCreateReward);
router.post('/tx-create-pay', TxCreatePay);

router.post('/handriseExperience', handriseExperience);
...
async function handriseExperience(req, res){
	try{
		const {id, exp, gameType} = req.body;
		var expSeckey, nameApp;
		if(!id && !exp && !gameType) return {status : false, message : "Not found userId"}
		if(gameType === 1) {
			expSeckey = process.env.RefeseckeyOddOven;
			nameApp = process.env.APP_ODDOVEN;
		}
		else if(gameType === 2){
			expSeckey = process.env.RefeseckeyRps;
			nameApp = process.env.APP_RPS;
		}
		else{
			expSeckey = process.env.RefeseckeyLotto;
			nameApp = process.env.APP_LOTTO;
		}

		var $ = require('../../lib/arguments').get({account: id, exp: exp, expType: 5});
		var rewardData = encryptor.encryptObjectBySeckey({account: $.account, exp: $.exp, expType: $.expType}, expSeckey);
		let rewardResult = await axios.post('http://213.136.68.203:41003/exp/exp-up',{app: nameApp, data: rewardData})
		// console.log('rewardResult',rewardResult);
		if(rewardResult.data.success) 
			return { status : true, Balance : rewardResult.data}
		return { status : false, message : "Invalid API call"}
	}
	catch(e){
		return { status : false, message : e}
	}
}
async function sipping(req, res){
	try{
			...
			endGameSchedule = schedule.scheduleJob('30 * * * * *', async function(){ //매 분 30초에 실행
				...
				for(let i=0;i<finishedGames.length;i++) {
					for(let j=0;j<3;j++) { //로또게임을 위해 블록 3개 저장
						...
						const hash_integer = await parseInt(getblock.block.hash.slice(-1),36) //해쉬 정수화
...
async function getTimestamp(req, res){
	try {
		res.json({timestamp:Date.now()+5000}) //서버 Timestamp + 5초 반환
	}
	catch(e) {
		console.log('Error : ', e);
	}
...
// 	try {
// 		const {idUser, tokenUser} = req.body;
// 		if(!idUser || !tokenUser) return { status : false, message : 'Not enough param'};

// 		var $ = require('../../lib/arguments').get({account: idUser, token: tokenUser}); // user_id to _user
// 		var data = encryptor.encryptObjectBySeckey({account: $.account, token: $.token}, APP_SECKEY);
// 		let balanceResult = await axios.post('http://167.99.69.209:7777/btamin/account-balance',{app: APP_NAME, data: data})
// 		if(balanceResult.data.success) {
// 			res.json({User: idUser,Balance : balanceResult.data.data.balance}) 
// 		} else {
// 			res.json({"err":"Invalid User_id"})
// 		}
// 	}
// 	catch(e) {
// 		console.log('Error : ', e);
...
async function userBalance(req, res){
	try {
		const {_user, _token} = req.body
		const $ = require('../../lib/arguments').get({account: _user, token: _token}); 
		const balanceData = encryptor.encryptObjectBySeckey({account: $.account, token: $.token}, APP_SECKEY);
		const balanceResult = await axios.post('http://167.99.69.209:7777/btamin/account-balance',{app: APP_NAME, data: balanceData})
		if(balanceResult.data.success) {
			res.json({balance : balanceResult.data.data.balance}) 
		} else {
			res.json({"err":"Invalid User_id"})
		}
	}
...
async function TxCreateReward(req, res, next) {
	try{
		const requestOption = {
			"amount" : 50000 ,
			"userId" : user_id
		}
		const $ = require('../../lib/arguments').get({amount: requestOption.amount, receiver: requestOption.user_id});
		const rewardData = encryptor.encryptObjectBySeckey({amount: $.amount, receiver: $.receiver}, APP_SECKEY);
		const rewardResult = await axios.post('http://167.99.69.209:7777/btamin/tx-create-reward',{app: APP_NAME, data: rewardData})
		if(rewardResult.data.success) {
			console.log('data' ,rewardResult.data)
		} else {
			res.json({"err":"Invalid User_id"})
		}
	}
...
async function TxCreatePay(req, res, next) {
	try{
		const requestOption = {
			"amount" : 50000 ,
			"userId" : user_id_LoiPham,
			"userToken" : user_token_LoiPham
		}
		$ = require('../../lib/arguments').get({amount: requestOption.amount, account: requestOption.user_id, token: requestOption.userToken}); // user_id : _user // user_token : _token
		const paymentData = encryptor.encryptObjectBySeckey({amount: $.amount, account: $.account, token: $.token}, APP_SECKEY);
		const paymentResult = await axios.post('http://167.99.69.209:7777/btamin/tx-create-pay',{app: APP_NAME, data: paymentData})
		if(paymentResult.data.success) {
